...
\lstdefinestyle{pyStyle}{
  ...
  morekeywords={Model,addVar,addCons,optimize,getVal,getObjVal,
                 getStatus,setMinimize,setMaximize,quicksum,
...
\begin{itemize}
  ...
  \item Solve classical optimization problems:
    \begin{itemize}
      \item Transportation, Blending, Set Cover,
...
\begin{column}{0.55\textwidth}
  ...
  \begin{itemize}
    \item One of the fastest non-commercial MIP solvers.
  ...
  \begin{itemize}
    \item Pythonic API for model building.
    \item Access to advanced features: callbacks, heuristics, separators, etc.
...
\medskip
Use \texttt{quicksum()} for efficient summation over index sets.
...
for j in customers:
    model.addCons(
        quicksum(x[i,j] for i in suppliers) == demand[j],
        name=f"demand_{j}"
    )

# quicksum is preferred over Python's sum() for performance
model.addCons(quicksum(c[i]*x[i] for i in I) <= b)
\end{lstlisting}
\end{frame}
...
Two ways to set the objective:

\medskip
\textbf{Option A:} Via \texttt{obj} parameter in \texttt{addVar}
\begin{lstlisting}
x = model.addVar(name="x", vtype="B", obj=3)  # coeff = 3
y = model.addVar(name="y", vtype="B", obj=2)  # coeff = 2
model.setMaximize()  # max 3x + 2y
\end{lstlisting}

\medskip
\textbf{Option B:} Via \texttt{setObjective}
\begin{lstlisting}
model.setObjective(
    quicksum(cost[i,j] * x[i,j] for i in I for j in J),
    sense="minimize"
)
\end{lstlisting}
...
Convenience methods:
\begin{itemize}
  \item \texttt{model.setMinimize()} / \texttt{model.setMaximize()}
\end{itemize}
\end{frame}
...
\begin{block}{Task}
Build and print the following model (do \textbf{not} solve yet):
\begin{align*}
  \max \quad & 3x + 2y \\
...
% ---------------------------------------------------------
% Frame 14: Blending Problem
% ---------------------------------------------------------
\begin{frame}{Blending Problem}
\begin{columns}[T]
\begin{column}{0.42\textwidth}
\begin{center}
\begin{tikzpicture}[
  raw/.style={rectangle,draw,rounded corners=2pt,minimum width=1.1cm,
              minimum height=0.6cm,font=\scriptsize\bfseries},
  mixer/.style={cylinder,draw,fill=gray!15,minimum width=1.2cm,
                minimum height=1.2cm,shape border rotate=90,
                font=\scriptsize\bfseries,aspect=0.4},
  output/.style={rectangle,draw,rounded corners=4pt,fill=green!15,
                 minimum width=1.3cm,minimum height=0.8cm,
                 font=\scriptsize\bfseries},
  arr/.style={-{Stealth},thick},
]
  % Raw materials
  \node[raw,fill=blue!20]   (m1) at (0, 1.6) {$m_1$};
  \node[raw,fill=red!20]    (m2) at (0, 0.5) {$m_2$};
  \node[raw,fill=orange!20] (m3) at (0,-0.6) {$m_3$};

  \node[anchor=south,font=\scriptsize] at (0,2.0) {Raw Materials};

  % Mixer
  \node[mixer] (mix) at (2.4,0.5) {Mixer};

  % Output
  \node[output] (out) at (4.6,0.5) {Blend};

  % Arrows with labels
  \draw[arr,blue!60]   (m1.east) -- node[above,font=\tiny]{$x_1$} (mix.150);
  \draw[arr,red!60]    (m2.east) -- node[above,font=\tiny]{$x_2$} (mix.west);
  \draw[arr,orange!60] (m3.east) -- node[below,font=\tiny]{$x_3$} (mix.210);
  \draw[arr,green!50!black,very thick] (mix.east) -- (out.west);

  % Quality annotation
  \node[below=0.3cm of out,font=\tiny,align=center,text=green!40!black]
    {Quality\\$\ell_q \le \cdot \le u_q$};
\end{tikzpicture}
\end{center}
\end{column}
\begin{column}{0.58\textwidth}
Combine raw materials to meet quality specs at minimum cost.

\smallskip
{\small\textbf{Sets:} materials $M$, quality attributes $Q$}

{\small\textbf{Parameters:} cost $c_m$, content $a_{mq}$,
bounds $[\ell_q, u_q]$, availability $b_m$}

{\small\textbf{Variables:} $x_m \ge 0$ (amount of material $m$)}

\begin{align*}
  \min \quad & \sum_{m \in M} c_m \, x_m \\
  \text{s.t.} \quad
    & \ell_q \le \frac{\sum_{m} a_{mq}\, x_m}{\sum_{m} x_m} \le u_q
      & \forall\, q \in Q \\
    & 0 \le x_m \le b_m & \forall\, m \in M
\end{align*}

{\small\textbf{Linearize:} $\ell_q \!\sum_{m} x_m \le \sum_{m} a_{mq} x_m \le u_q \!\sum_{m} x_m$}
\end{column}
\end{columns}
\end{frame}
...
% Frame 15: Exercise 4
% ---------------------------------------------------------
\begin{frame}{Exercise 4: Blending}
\begin{block}{Task}
Implement the blending problem:
\begin{itemize}
  \item Model quality bounds as linear constraints (after linearization).
  \item Handle material availability limits.
...
\begin{center}
\begin{tikzpicture}[
  elem/.style={circle,fill=black,inner sep=1.5pt},
  elabel/.style={font=\tiny,above=1pt},
]
  % Elements (dots)
  \node[elem,label={[elabel]above:1}] (e1) at (0.4, 1.2) {};
  \node[elem,label={[elabel]above:2}] (e2) at (1.3, 1.5) {};
  \node[elem,label={[elabel]above:3}] (e3) at (2.3, 1.3) {};
  \node[elem,label={[elabel]below:4}] (e4) at (0.7, 0.2) {};
  \node[elem,label={[elabel]below:5}] (e5) at (1.8, 0.0) {};
  \node[elem,label={[elabel]right:6}] (e6) at (3.0, 0.5) {};
  \node[elem,label={[elabel]above:7}] (e7) at (3.2, 1.6) {};

  % Sets as ellipses
  \draw[thick,fill=blue!12,draw=blue!60,rounded corners]
    (0.1,0.6) ellipse (0.9cm and 0.9cm);
  \node[font=\scriptsize,blue!70] at (-0.4,0.0) {$S_1$};

  \draw[thick,fill=red!12,draw=red!60,rounded corners]
    (1.6,1.1) ellipse (1.1cm and 0.8cm);
  \node[font=\scriptsize,red!70] at (1.6,1.9) {$S_2$};

  \draw[thick,fill=green!12,draw=green!60!black,rounded corners]
    (2.0,-0.1) ellipse (0.8cm and 0.6cm);
  \node[font=\scriptsize,green!50!black] at (2.0,-0.7) {$S_3$};

  \draw[thick,fill=orange!12,draw=orange!70,rounded corners]
    (3.1,1.0) ellipse (0.6cm and 0.9cm);
  \node[font=\scriptsize,orange!70] at (3.7,1.0) {$S_4$};
...
\end{center}
\vspace{-2pt}
{\centering\scriptsize Select fewest sets to cover all elements.\par}
\end{column}
\begin{column}{0.58\textwidth}
...
\smallskip
\begin{itemize}
  \item NP-hard in general; SCIP handles it efficiently via specialized techniques.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{block}{Task}
Implement the capacitated facility location problem:
\begin{itemize}
