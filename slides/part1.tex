...
\begin{center}
\begin{tikzpicture}[
  step/.style={draw,rounded corners,minimum width=2.2cm,
               minimum height=0.7cm,align=center,font=\small\bfseries,
               fill=blue!10},
  arr/.style={-{Stealth},thick,shorten >=2pt,shorten <=2pt},
]
  \node[step] (create)  {Create Model};
  \node[step,right=1.0cm of create]  (vars)  {Add Variables};
  \node[step,right=1.0cm of vars]    (cons)  {Add Constraints};
  \node[step,right=1.0cm of cons]    (opt)   {Optimize};
  \node[step,right=1.0cm of opt]     (query) {Query Solution};
...
Combine raw materials to meet quality specifications at minimum cost.

\medskip
\textbf{Sets:} materials $M$, quality attributes $Q$

\textbf{Parameters:} cost $c_m$, attribute content $a_{mq}$,
bounds $[\ell_q, u_q]$, availability $b_m$

\textbf{Variables:} $x_m \ge 0$ (amount of material $m$)
...
\medskip
\textbf{Linearize} by multiplying through by $\sum_m x_m$:
\[
  \ell_q \sum_{m} x_m \;\le\; \sum_{m} a_{mq}\, x_m
  \;\le\; u_q \sum_{m} x_m
\]
\end{frame}
...
\medskip
\textbf{Applications:} budgeting, cargo loading, resource allocation.
...
where $v_j$ = value, $w_j$ = weight, $W$ = capacity.
...
\medskip
\textbf{Sets:} facilities $F$, customers $C$

\textbf{Parameters:} fixed cost $f_i$, assignment cost $t_{ij}$, demand $d_j$, capacity $q_i$

\textbf{Variables:} $y_i \in \{0,1\}$ (open $i$?), \; $x_{ij} \ge 0$ (fraction served)
...
\medskip
\textbf{Variables:} $x_{v,c} \in \{0,1\}$ (vertex $v$ gets color $c$?), \;
...
\medskip
\begin{itemize}
  \item \textbf{Symmetry:} any permutation of colors gives an equivalent
        solution --- this makes MIP solving harder.
  \item Break symmetry: $w_1 \ge w_2 \ge \cdots \ge w_k$.
\end{itemize}
\end{frame}
