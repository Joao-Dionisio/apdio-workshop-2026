...
\titlegraphic{\includegraphics[height=1.5cm]{FIGS/cmup-info.png}}
\hypersetup{
...
\label{sec:orgd6eb28a}

\begin{frame}[label={sec:org2017199}]{Background}
\begin{itemize}
...
\begin{frame}[label={sec:orgb3d48dc}]{The Pre-History: Joseph Fourier (1826)}
...
\begin{frame}[label={sec:org4b22c02}]{Comparing "Projecting" vs. "Walking"}
\begin{itemize}
\item Fourier (1826):
\begin{itemize}
...
\begin{frame}[label={sec:org18fa8a1}]{Visualizing the Two Approaches}
...
\begin{frame}[label={sec:org71e0553}]{Fourier's Method "Explodes"}
\begin{itemize}
...
\begin{frame}[label={sec:org4fe396f}]{The Stigler Diet Problem (1947)}
...
\begin{frame}[label={sec:org393dff1}]{The Human "Computer": Jack Laderman's Achievement}
...
\begin{frame}[label={sec:org8d94fb6}]{First Machine Implementations: SEAC and IBM 701}
...
\begin{frame}[label={sec:org5a9b0ab},fragile]{Before Modeling Languages: Physical Data Entry}
...
\begin{frame}[label={sec:org30f31d5}]{Before Modeling Languages: Workflow}
\begin{enumerate}
...
\begin{frame}[label={sec:orgff246a0}]{The "Middle Ages": Matrix Generators}
...
\begin{frame}[label={sec:org6368686},fragile]{The "Middle Ages"}
...
\begin{frame}[label={sec:org2a7336d},fragile]{The MPS Format (The Lingua Franca)}
...
\begin{frame}[label={sec:org358b6ed},fragile]{MPS Format}
...
\begin{frame}[label={sec:orge2e128f},fragile]{The Representation Gap}
...
\begin{frame}[label={sec:orgcf0ca8c},fragile]{MPS: "Technical Debt" of the 1970s}
 \begin{itemize}
...
\label{sec:org535ccfb}

\begin{frame}[label={sec:org9f6d112}]{Model Taxonomy: Choosing the Right Solver}
...
\label{sec:orgc8a06dd}
\begin{frame}[label={sec:org1e4c20c},fragile]{The Renaissance: GAMS (1976)}
...
\begin{frame}[label={sec:org5eac1e6}]{The "good" modeling language checklist}
\begin{enumerate}
...
\begin{frame}[label={sec:org16e3c1c},fragile]{Solver Independence}
 \begin{itemize}
...
\label{sec:orgbd87158}
\begin{frame}[label={sec:orgda30cb3}]{Modern usage}
\begin{itemize}
...
\begin{frame}[label={sec:orge3628b7}]{}
...
\begin{frame}[label={sec:orgcaf948b},fragile]{Syntax Showdown: AMPL vs. Pyomo}
...
\begin{frame}[label={sec:org467223f},fragile]{Modern Usage}
...
\begin{frame}[label={sec:org57c4a86},fragile]{The Unified Syntax: amplpy}
...
\begin{frame}[label={sec:org8cb18bb}]{Modern Tech Stack: A Modeler's Compass}
...
\begin{frame}[label={sec:orgd2c0d55},fragile]{The Evolution of the Modeling Interface}
...
\begin{frame}[label={sec:orgb065aea},fragile]{Advanced Topics: Column Generation}
...
\begin{frame}[label={sec:orga8ccfc8},fragile]{Advanced Topics: Cutting Planes}
...
\begin{block}{Advantage of the API}
\begin{itemize}
\item No need to restart the solver from scratch.
\item The solver (Gurobi/CPLEX/\ldots{}) performs a \alert{\alert{warm start}}, keeping the previous basis.
\item This capability is only possible via memory-resident APIs (\texttt{amplpy}), not file-based MPS.
\end{itemize}
\end{block}
\end{frame}
...
\label{sec:orgd1bf125}

\begin{frame}[label={sec:orgb55d0ab}]{Real-World Deployment: Beyond the Notebook}
...
\item \alert{\alert{Predictive}}: ML models (Scikit-learn/XGBoost) predict parameters (e.g., future demand).
\item \alert{\alert{Prescriptive}}: AMPL/Python solves for the optimal plan based on those predictions.
...
\begin{frame}[label={sec:orgc504e94},fragile]{Conclusion: From Alpha to Omega}
 \begin{block}{The Journey of Abstraction}
\begin{itemize}
\item \alert{\alert{The Alpha}}: Mathematical truth remains constant (Dantzig's Simplex).
...
\item \alert{\alert{The Omega}}: The Unified API (DSL + GPL).
\end{itemize}
\end{block}
...
\begin{frame}[label={sec:org0a2c5ef}]{Beyond the Black Box}
\begin{block}{The Limit of High-Level APIs}
\begin{itemize}
\item \alert{High-Level}: send a model \(\to\) get a solution.
\item No concerns on \emph{how to solve}
\item In some cases, we must dig into the solution method:
\begin{itemize}
\item Dantzig-Wolfe decomposition: add variables \alert{during} the branch-and-bound search.
\end{itemize}
\item Standard APIs usually require \emph{stopping the solver, adding a column, and re-starting.}
\begin{itemize}
\item this destroys the solver's internal state
\end{itemize}
\end{itemize}
\end{block}

\begin{block}{SCIP: The "Transparent" Solver}
\begin{itemize}
\item Allows writing a \alert{\alert{pricer}} in C or Python \(\to\) called \alert{internally} every time a node is processed.
\item Can manage the \emph{master and subproblem} coordination natively without ever exiting the search process.
\item Next: \alert{Moving from "What to solve" to "How to solve."}
\end{itemize}
\end{block}
\end{frame}


\begin{frame}[label={sec:org7493cf4}]{Further Reading and References: The "Alpha" (Foundational Papers)}
\begin{itemize}
...
\begin{frame}[label={sec:org3303491}]{Further Reading and References: The "Middle Ages" and "Renaissance"}
\begin{itemize}
...
\begin{frame}[label={sec:orge5af274},fragile]{The "Omega": Modern APIs and Libraries}
 \begin{itemize}
